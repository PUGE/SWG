...
// 切换页面特效
switchPage(oldUrlParam,newUrlParam);};window.ozzx={script:{"home":{"data":{"mode":"real","output":"background","outText":false},"created":function created(){var _this=this;$dom('uploadInput').onchange=function(){_this.upload($dom('uploadInput').files[0]);};// 拖拽上传事件
var uploadBox=$dom('uploadBox');uploadBox.ondragcenter=function(e){e.preventDefault();};uploadBox.ondragover=function(e){e.preventDefault();uploadBox.classList.add('hover');};uploadBox.ondragleave=function(e){e.preventDefault();e.stopImmediatePropagation();uploadBox.classList.remove('hover');};uploadBox.ondrop=function(e){e.preventDefault();uploadBox.classList.remove('hover');_this.upload(e.dataTransfer.files[0]);};},"uploadFile":function uploadFile(){this.$event.preventDefault();$dom('uploadInput').click();},"upload":function upload(fileData){if(!fileData){console.log('放弃上传!');}if(fileData.name.lastIndexOf('.psd')+4!==fileData.name.length){alert('只允许上传psd文件!');return false;}if(fileData.size>5*60*1024*1024){alert('上传文件的大小超出限制!');return false;}var file=new FormData();file.append('data',fileData);// 获取选择模式
var mode=this.data.mode;// 输出模式
var output=this.data.output;var outText=this.data.outText;// 传输文件
var xhr=new XMLHttpRequest();xhr.open("POST",'uploads?mode='+mode+'&outText='+outText+'&output='+output,true);//上传进度事件
xhr.upload.addEventListener("progress",function(result){if(result.lengthComputable){var percentComplete=result.loaded/result.total;$dom('stareIcon').style.display='none';$dom('progress').style.display='block';$dom('progress').value=Math.round(percentComplete*100);}},false);xhr.addEventListener("readystatechange",function(){var result=xhr;if(result.status!=200){//error
console.log('上传失败',result.status,result.statusText,result.response);}else if(result.readyState==4){// 上传完毕
var msg=JSON.parse(result.response);$go('show','moveToLeft','moveFromRight',{id:msg.id});}});xhr.send(file);},"changeModle":function changeModle(){if(this.$el.checked){this.data.mode=this.$el.value;}},"changeOutPut":function changeOutPut(){if(this.$el.checked){this.data.output=this.$el.value;}},"changeOutText":function changeOutText(item){this.data[item]=!this.data[item];}},"show":{"data":{},"created":function created(){// 获取url参数
