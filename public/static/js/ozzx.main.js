...
// 切换页面特效
switchPage(oldUrlParam,newUrlParam);};window.ozzx={script:{"home":{"data":{"mode":"currency","output":"background","outText":false,"compress":true,"switchMode":"vertical","adaptation":"real","bgm":false,"musicSrc":"https://cunchu.site/swg/bgm.mp3"},"created":function created(){var _this=this;$dom('uploadInput').onchange=function(){_this.upload($dom('uploadInput').files[0]);};// 拖拽上传事件
var uploadBox=$dom('uploadBox');uploadBox.ondragcenter=function(e){e.preventDefault();};uploadBox.ondragover=function(e){e.preventDefault();uploadBox.classList.add('hover');};uploadBox.ondragleave=function(e){e.preventDefault();e.stopImmediatePropagation();uploadBox.classList.remove('hover');};uploadBox.ondrop=function(e){e.preventDefault();uploadBox.classList.remove('hover');_this.upload(e.dataTransfer.files[0]);};},"uploadFile":function uploadFile(){this.$event.preventDefault();$dom('uploadInput').click();},"upload":function upload(fileData){if(!fileData){console.log('放弃上传!');}if(fileData.name.lastIndexOf('.psd')+4!==fileData.name.length){alert('只允许上传psd文件!');return false;}if(fileData.size>5*60*1024*1024){alert('上传文件的大小超出限制!');return false;}var file=new FormData();file.append('data',fileData);for(var key in this.data){file.append(key,this.data[key]);}// 获取选择模式
var mode=this.data.mode;// 输出模式
var output=this.data.output;var outText=this.data.outText;// 传输文件
var xhr=new XMLHttpRequest();xhr.open("POST",'uploads',true);//上传进度事件
xhr.upload.addEventListener("progress",function(result){if(result.lengthComputable){var percentComplete=result.loaded/result.total;$dom('stareIcon').style.display='none';$dom('progress').style.display='block';var uploadValue=Math.round(percentComplete*100);if(uploadValue<100){$dom('progressText').innerText="\u6B63\u5728\u4E0A\u4F20\u6587\u4EF6: ".concat(uploadValue,"%");}else{$dom('progressText').innerText="\u670D\u52A1\u5668\u6B63\u5728\u5BF9PSD\u6587\u4EF6\u8FDB\u884C\u5904\u7406!";}}},false);xhr.addEventListener("readystatechange",function(){var result=xhr;if(result.status!=200){//error
console.log('上传失败',result.status,result.statusText,result.response);}else if(result.readyState==4){// 上传完毕
...
$dom('show').src='./temp/'+$tool.getQueryString('id');$dom('show').style.display='block';setTimeout(function(){new QRCode(document.getElementById("qr"),{text:"http://"+window.location.host+'/temp/'+$tool.getQueryString('id'),width:150,height:150,colorDark:"#000000",colorLight:"#ffffff",correctLevel:QRCode.CorrectLevel.H});},500);// 绑定点击事件
document.getElementById('show').contentDocument.onclick=function(){console.log('sdsd');};},"down":function down(){var httpRequest=new XMLHttpRequest();httpRequest.open('GET','down?id='+$tool.getQueryString('id'),true);httpRequest.send();/**
       * 获取数据后的处理程序
...
function animation(oldDom,newDom,animationIn,animationOut){// 获取父元素
var parentDom=newDom.parentElement;if(!oldDom){console.error('旧页面不存在!');}oldDom.addEventListener("animationend",oldDomFun);newDom.addEventListener("animationend",newDomFun);oldDom.style.position='absolute';newDom.style.display='block';newDom.style.position='absolute';// document.body.style.overflow = 'hidden'
parentDom.style.perspective='1200px';oldDom.classList.add('ozzx-animation');animationIn.split(',').forEach(function(value){oldDom.classList.add('ox-page-'+value);});newDom.classList.add('ozzx-animation');animationOut.split(',').forEach(function(value){newDom.classList.add('ox-page-'+value);});// 旧DOM执行函数
function oldDomFun(){// 移除监听
oldDom.removeEventListener('animationend',oldDomFun,false);// 隐藏掉旧的节点
oldDom.style.display='none';// console.log(oldDom)
oldDom.style.position='';oldDom.classList.remove('ozzx-animation');parentDom.style.perspective='';// 清除临时设置的class
animationIn.split(',').forEach(function(value){oldDom.classList.remove('ox-page-'+value);});}// 新DOM执行函数
function newDomFun(){// 移除监听
newDom.removeEventListener('animationend',newDomFun,false);// 清除临时设置的style
newDom.style.position='';newDom.classList.remove('ozzx-animation');animationOut.split(',').forEach(function(value){newDom.classList.remove('ox-page-'+value);});}}// 切换页面前的准备工作
function switchPage(oldUrlParam,newUrlParam){var oldPage=oldUrlParam;var newPage=newUrlParam;var newPagParamList=newPage.split('&');if(newPage)newPage=newPagParamList[0];// 查找页面跳转前的page页(dom节点)
// console.log(oldUrlParam)
// 如果源地址获取不到 那么一般是因为源页面为首页
if(oldPage===undefined){oldPage=ozzx.entry;}else{oldPage=oldPage.split('&')[0];}var oldDom=document.getElementById('ox-'+oldPage);var newDom=document.getElementById('ox-'+newPage);if(!newDom){console.error('页面不存在!');return;}// 判断是否有动画效果
if(!ozzx.state.animation)ozzx.state.animation={};var animationIn=ozzx.state.animation.in;var animationOut=ozzx.state.animation.out;if(animationIn||animationOut){// 如果没用动画参数则使用默认效果
if(!animationIn||!animationOut){dispalyEffect(oldDom,newDom);return;}ozzx.state.animation={};animation(oldDom,newDom,animationIn,animationOut);}else{dispalyEffect(oldDom,newDom);}window.ozzx.activePage=newPage;// 更改$data链接
